...
const HostelDetails = ({ hostelInfo }) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `https://api.geoapify.com/v1/geocode/reverse?lat=${hostelInfo.location.coordinates[1]}&lon=${hostelInfo.location.coordinates[0]}&format=json&apiKey=${API_KEY}`
      )
      .then((res) => {
        setHostelAddress(
          `${res.data.results[0].street}, ${res.data.results[0].city}, ${res.data.results[0].country}`
        );
      })
      .catch((err) => {
        console.log(err);
  ...
  return (
    <Wrapper>
      ...
      {/* <h2>{hostelAddress}</h2> */}
      <p>
        {hostelInfo.amenities[0]} • {hostelInfo.amenities[1]} •{' '}
        {hostelInfo.amenities[2]} • {hostelInfo.amenities[3]}
      </p>
      <TabsWrapper>
        <Tabs>
          {openDescriptionOption ? (
            <ActiveTab>Description</ActiveTab>
          ) : (
            <p onClick={() => handleActiveTab('d')}>Description</p>
          )}
          {openOfferOption ? (
            <ActiveTab>What we offer</ActiveTab>
          ) : (
            <p onClick={() => handleActiveTab('w')}>What we offer</p>
          )}
          {openReviewOption ? (
